...
addprocs(4)  # Adjust the number based on how many workers you want to use
...
@everywhere begin
    ...
    function run_simulations(T_array::Vector{Float64}, constant_inputs::OrderedDict{Any, Any}, variable_inputs::OrderedDict{Any, Any}, sys_in::String, output_dir::String = nothing) :: Dict
        ...
        combinations = collect(IterTools.product(values(new_variable_inputs)...))  # All combinations of variable_inputs
        println("Performing $(length(combinations)) fractional crystallisation simulations...")

        # Use a shared array to store results from parallel executions
        # Initialize a results dictionary that will be updated in parallel
        results = Dict{String, Any}()  # Dictionary to store simulation results
        lock = ReentrantLock()  # Use lock to ensure thread safety when updating `results`

        # Use pmap to distribute the combinations across workers
        pmap_result = pmap(combination -> begin
        ...
        # Store results from pmap in the dictionary
        for (output_file, output_data) in pmap_result
            results[output_file] = output_data
        end
