...
@testset "Check constant_inputs values are numeric" begin
    ...
    @test MAGEMinEnsemble.InputValidation.check_constant_inputs_values(constant_inputs_valid) === nothing
    ...
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_constant_inputs_values(constant_inputs_invalid)

    # Test Case 4: "buffer" is not a string
    constant_inputs_invalid_buffer = OrderedDict("SiO2" => 53.0, "FeO" => 10.0, "buffer" => 123)
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_constant_inputs_values(constant_inputs_invalid_buffer)
end
...
@testset "Check variable_inputs values are vectors" begin
    ...
    @test MAGEMinEnsemble.InputValidation.check_variable_inputs_vectors(variable_inputs_valid) === nothing

    # Test Case 2: non-vector value
    variable_inputs_invalid = OrderedDict("P" => "300,400")
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_variable_inputs_vectors(variable_inputs_invalid)
end

@testset "check_keys_oxygen" begin
    # Test case 1: Dictionary with keys containing "O"
    od = OrderedDict("SiO2" => 10.0, "TiO2" => 5.0, "Al2O3" => 7.0)
    @test MAGEMinEnsemble.InputValidation.check_keys_oxygen(od) == ["SiO2", "TiO2", "Al2O3"]  # All the keys contain "O"

    # Test case 2: Dictionary with no keys containing "O"
    od = OrderedDict("P" => 5.0)
    @test MAGEMinEnsemble.InputValidation.check_keys_oxygen(od) == []  # No keys contain "O"

    # Test case 3: Dictionary with some keys containing "O"
    od = OrderedDict("P" => 5.0, "SiO2" => 3.0, "FeO" => 2.0)
    @test MAGEMinEnsemble.InputValidation.check_keys_oxygen(od) == ["SiO2", "FeO"]  # Only SiO2 and FeO contain "O"

    # Test case 5: Empty dictionary (no keys)
    od = OrderedDict()
    @test MAGEMinEnsemble.InputValidation.check_keys_oxygen(od) == []  # No oxides as dictionary is empty

    # Test case 6: Dictionary with "O" in some keys, including "O" alone
    od = OrderedDict("O" => 2.0, "SiO2" => 10.0, "CaO" => 8.0)
    @test MAGEMinEnsemble.InputValidation.check_keys_oxygen(od) == ["O", "SiO2", "CaO"]  # "O" should be included
end
...
@testset "validate_oxides" begin
    ...
    @test MAGEMinEnsemble.InputValidation.validate_oxides(constant_inputs, variable_inputs) === nothing
    ...
    variable_inputs = OrderedDict()
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.validate_oxides(constant_inputs, variable_inputs)
...
@testset "Key validation" begin
    ...
    @test MAGEMinEnsemble.InputValidation.check_matching_keys(constant_inputs, variable_inputs) === nothing
    ...
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.check_matching_keys(constant_inputs_with_common_keys, variable_inputs_with_common_keys)
end
...
@testset "validate_positive_pressure" begin
    ...
    variable_inputs = OrderedDict()
    @test MAGEMinEnsemble.InputValidation.validate_positive_pressure(constant_inputs, variable_inputs) === nothing
    ...
    variable_inputs = OrderedDict()
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.validate_positive_pressure(constant_inputs, variable_inputs)
...
@testset "validate_positive_oxides" begin
    ...
    variable_inputs = OrderedDict()
    @test MAGEMinEnsemble.InputValidation.validate_positive_oxides(constant_inputs, variable_inputs) === nothing
    ...
    constant_inputs = OrderedDict("SiO2" => -10.0, "TiO2" => 5.0)
    variable_inputs = OrderedDict()
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.validate_positive_oxides(constant_inputs, variable_inputs)
...
@testset "validate_buffer tests" begin
    ...
    @test MAGEMinEnsemble.InputValidation.validate_buffer(constant_inputs, variable_inputs) === nothing
    ...
    variable_inputs = OrderedDict()  # No buffer here
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.validate_buffer(constant_inputs, variable_inputs)
...
@testset "check_buffer_if_offset" begin
    ...
    variable_inputs = OrderedDict()
    @test MAGEMinEnsemble.InputValidation.check_buffer_if_offset(constant_inputs, variable_inputs) === nothing
    ...
    constant_inputs = OrderedDict("offset" => 1.0)
    variable_inputs = OrderedDict()
    @test_throws ErrorException MAGEMinEnsemble.InputValidation.check_buffer_if_offset(constant_inputs, variable_inputs)
...
@testset "replace_zero_oxides" begin
    ...
    variable_inputs = OrderedDict()
    new_constant_inputs, new_variable_inputs = MAGEMinEnsemble.InputValidation.replace_zero_oxides(constant_inputs, variable_inputs)
...
@testset "check_required_inputs" begin
    ...
    variable_inputs = OrderedDict()
    @test MAGEMinEnsemble.InputValidation.check_required_inputs(constant_inputs, variable_inputs) === nothing
    ...
    @test_throws ErrorException("Required input 'T_step' is not present in either constant_inputs or variable_inputs.") MAGEMinEnsemble.InputValidation.check_required_inputs(constant_inputs, variable_inputs)
end

@testset "check_bulk_frac" begin
    # Valid inputs
    @test MAGEMinEnsemble.InputValidation.check_bulk_frac("bulk") == nothing
    @test MAGEMinEnsemble.InputValidation.check_bulk_frac("frac") == nothing

    # Invalid inputs: Should throw an ArgumentError
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_bulk_frac("other")
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_bulk_frac("invalid_value")
end


@testset "check_td_database" begin
    # Valid inputs
    @test MAGEMinEnsemble.InputValidation.check_td_database("mtl") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("mp") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("mb") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("ig") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("igad") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("um") == nothing
    @test MAGEMinEnsemble.InputValidation.check_td_database("ume") == nothing

    # Invalid inputs: Should throw an ArgumentError
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_td_database("other")
    @test_throws ArgumentError MAGEMinEnsemble.InputValidation.check_td_database("invalid_database")
end